...
DatabaseManager::RefreshTokenResult DatabaseManager::refreshAuthToken(const std::string& old_token)
{
    ...
    try {
        // 1. Проверяем существование, валидность и срок истечения старого токена
        std::string checkSql =
            ...
            "WHERE t.token = ? "
            "AND u.is_active = 1 "
            "AND TIMESTAMPDIFF(MINUTE, NOW(), t.expires_at) BETWEEN 1 AND 60"; // Осталось 1-60 минут
        ...
        // Токен не найден, пользователь неактивен или срок не подходит для обновления
        if (!checkResult.next()) {
            // Уточняем причину для лучшего сообщения об ошибке
            std::string detailedCheckSql =
                "SELECT t.expires_at, t.is_suspicious, u.is_active "
                "FROM tokens t "
                "JOIN users u ON t.user_id = u.id "
                "WHERE t.token = ?";

            nanodbc::statement detailedStmt(*connection_);
            nanodbc::prepare(detailedStmt, detailedCheckSql);
            detailedStmt.bind(0, old_token.c_str());
            nanodbc::result detailedResult = detailedStmt.execute();

            if (!detailedResult.next()) {
                result.error_msg = "Token not found";
            } else {
                // Токен найден, проверяем детали
                int is_suspicious = 0;
                int user_active = 0;
                std::string expires_at_str;

                if (!detailedResult.is_null(0)) expires_at_str = detailedResult.get<std::string>(0);
                if (!detailedResult.is_null(1)) is_suspicious = detailedResult.get<int>(1);
                if (!detailedResult.is_null(2)) user_active = detailedResult.get<int>(2);

                if (is_suspicious == 1) {
                    result.error_msg = "Token marked as suspicious, cannot refresh";
                } else if (user_active == 0) {
                    result.error_msg = "User account is inactive";
                } else {
                    // Проверяем срок действия
                    std::string timeCheckSql =
                        "SELECT TIMESTAMPDIFF(MINUTE, NOW(), expires_at) "
                        "FROM tokens WHERE token = ?";

                    nanodbc::statement timeStmt(*connection_);
                    nanodbc::prepare(timeStmt, timeCheckSql);
                    timeStmt.bind(0, old_token.c_str());
                    nanodbc::result timeResult = timeStmt.execute();

                    if (timeResult.next()) {
                        int minutes_left = timeResult.get<int>(0);
                        if (minutes_left <= 0) {
                            result.error_msg = "Token has expired";
                        } else if (minutes_left > 60) {
                            result.error_msg = "Token can only be refreshed when 60 minutes or less remain. Current: " +
                                               std::to_string(minutes_left) + " minutes";
                        } else {
                            result.error_msg = "Token refresh conditions not met";
                        }
                    } else {
                        result.error_msg = "Cannot determine token expiration";
                    }
                }
            }
            return result;
...
DatabaseManager::ChangePasswordResult DatabaseManager::resetUserPassword(
    int target_user_id, const std::string& new_password)
{
    std::lock_guard<std::mutex> lock(authMutex_);
    ChangePasswordResult result;
    ...
    if (target_user_id <= 0 || new_password.empty()) {
        result.error_msg = "Invalid parameters";
        return result;
    }
    ...
    try {
        // 1. Проверяем существование пользователя и получаем его логин
        std::string checkSql = "SELECT login FROM users WHERE id = ? AND is_active = 1";
        nanodbc::statement checkStmt(*connection_);
        nanodbc::prepare(checkStmt, checkSql);
        checkStmt.bind(0, &target_user_id, 1);
        ...
        std::string user_login;
        if (!checkResult.is_null(0)) {
            user_login = checkResult.get<std::string>(0);
        }

        // 2. ДОПОЛНИТЕЛЬНАЯ ПРОВЕРКА: Не допускать пароли, содержащие логин
        if (!user_login.empty()) {
            std::string lower_password = new_password;
            std::string lower_login = user_login;
            ...
            if (lower_password.find(lower_login) != std::string::npos) {
                result.error_msg = "Password should not contain user's login";
                return result;
            }
        }

        // 3. Генерируем новый хеш
        std::string new_hash = PasswordHasher::hashPassword(new_password);

        // 4. Извлекаем соль из нового хеша
        size_t pos1 = new_hash.find(':');
        size_t pos2 = new_hash.find(':', pos1 + 1);
        size_t pos3 = new_hash.find(':', pos2 + 1);
        ...
        updateStmt.bind(0, new_hash.c_str());
        updateStmt.bind(1, new_salt.c_str());
        updateStmt.bind(2, &target_user_id, 1);

        updateStmt.execute();

        // 6. Удаляем все активные токены пользователя
        std::string deleteTokensSql = "DELETE FROM tokens WHERE user_id = ?";
        nanodbc::statement tokenStmt(*connection_);
        nanodbc::prepare(tokenStmt, deleteTokensSql);
        tokenStmt.bind(0, &target_user_id, 1);
        tokenStmt.execute();
