# Guarder Server

## 📌 Общая архитектура системы

### 1.1. Компоненты системы

**ServerCore — ядро HTTP/HTTPS сервера**  
- **Назначение:** обработка входящих HTTP/HTTPS запросов, маршрутизация, управление сессиями  
- **Технологии:** библиотека [civetweb](https://github.com/civetweb/civetweb)  
- **Особенности:**
  - Поддержка только HTTPS (HTTP порт отключён)
  - HSTS заголовки для принудительного использования HTTPS
  - Security headers (XSS-Protection, Clickjacking protection)
  - Rate limiting на аутентификацию

**DatabaseManager — менеджер базы данных**  
- **Назначение:** взаимодействие с БД, аутентификация, управление пользователями  
- **Технологии:** [nanodbc](https://github.com/nanodbc/nanodbc), MySQL/PostgreSQL через ODBC DSN  
- **Особенности:**
  - Потокобезопасность (мьютексы для разных операций)
  - Криптографически безопасные токены
  - Отслеживание подозрительной активности (IP/User-Agent изменения)
  - Автоматическая очистка устаревших токенов

**PasswordHasher — модуль работы с паролями**  
- **Назначение:** безопасное хеширование и валидация паролей  
- **Технологии:** OpenSSL PBKDF2-HMAC-SHA256  
- **Особенности:**
  - Проверка сложности паролей (8+ символов, регистры, цифры, спецсимволы)
  - Защита от распространённых слабых паролей
  - Генерация безопасных случайных паролей

---

### 1.2. Схема взаимодействия

```
Клиент (POSTMAN/браузер)
        ↓
[ HTTPS → Port 8443 ]
        ↓
   ServerCore
        ↓
   (Маршрутизация)
        ↓
[Аутентификация → DatabaseManager]
        ↓
[Валидация пароля → PasswordHasher]
        ↓
   [Обработка запроса]
        ↓
   [Ответ в JSON]
```

---

### 1.3. Технологический стек

- **Язык:** C++17  
- **Библиотеки:**
  - civetweb — встроенный веб-сервер
  - nanodbc — работа с БД через ODBC
  - OpenSSL — криптография (PBKDF2, SHA256)
  - JSONCpp — парсинг и генерация JSON
  - Qt — только для GUI (тестовый интерфейс)
- **База данных:** любая с ODBC драйвером (MySQL, PostgreSQL, SQL Server)  
- **DSN:** `GuarderDB`  
- **Протоколы:** HTTPS (TLS 1.2/1.3), REST API (POST + JSON)

---

### 1.4. Принципы проектирования

- **Безопасность:**
  - PBKDF2 с уникальной солью
  - Ограниченный срок жизни токенов (24 часа)
  - Проверка IP/User-Agent
  - SQL-инъекции предотвращаются параметризованными запросами
  - XSS защита (экранирование HTML в JSON)

- **Надёжность:**
  - Connection pool
  - Автовосстановление соединений
  - Логирование критических операций
  - Graceful shutdown

- **Масштабируемость:**
  - Один экземпляр DatabaseManager
  - Потокобезопасность через мьютексы
  - Возможность запуска нескольких экземпляров сервера

---

### 1.5. Директории проекта

```
project/
├── src/
│   ├── servercore.cpp/h       # Ядро сервера
│   ├── databasemanager.cpp/h  # Менеджер БД
│   ├── passwordhasher.cpp/h   # Хеширование паролей
│   └── civetweb/              # Встроенный веб-сервер
├── ssl_certs/
│   └── server.pem             # SSL сертификат
├── gui/                       # Тестовый GUI (Qt)
└── sql/
    └── create_tables.sql      # Структура БД
```

---

### 1.6. Жизненный цикл запроса

1. Получение HTTPS запроса на порт 8443  
2. Извлечение токена из заголовка `Authorization: Bearer`  
3. Валидация токена через DatabaseManager  
4. Проверка прав доступа по роли пользователя  
5. Обработка бизнес-логики  
6. Формирование JSON ответа с security заголовками  
7. Логирование операции  

---

### 1.7. Ключевые особенности

- Только HTTPS — HTTP отключён  
- Self-contained — минимальные внешние зависимости  
- Кросс-платформенность — Windows и Linux  
- Легковесность — один исполняемый файл + сертификат  
- Простая конфигурация — DSN и сертификат рядом с .exe  

---

### 1.8. SSL сертификат

- **Формат:** PEM  
- **Имя файла:** `server.pem`  
- **Размещение:** рядом с исполняемым файлом или в `ssl_certs/`  
- **Проверка при старте:** вывод ошибки, если файл не найден  

---

## Установка и настройка базы данных (MySQL + ODBC)

Для работы модуля **DatabaseManager** требуется настроенная база данных и ODBC-драйвер.

### 1. Создание базы данных
- SQL-скрипт для создания структуры БД находится по пути:  
  ```
  data/Создание БД GUARDER с таблицами.sql
  ```
- Выполните скрипт в MySQL, чтобы создать базу данных `guarder_base` и необходимые таблицы.

### 2. Установка ODBC драйвера
- Скачайте и установите **MySQL Connector/ODBC 9.5.0 ** с официального сайта MySQL.  
- После установки откройте утилиту **ODBC Data Sources**.

### 3. Настройка System DSN
В разделе **System DSN** добавьте новый источник данных:

- **Driver:** MySQL ODBC 9.5 Unicode Driver  
- **Data Source Name:** `GuarderDB`  
- **TCP/IP Server:** `localhost:3306`  
- **User:** `root`  
- **Password:** пароль, заданный при установке MySQL  
- **Database:** `guarder_base`

После сохранения DSN сервер будет использовать подключение `GuarderDB` для работы с базой данных.

## Установка и настройка OpenSSL (зависимости)

Для работы модуля **PasswordHasher** требуется установленный OpenSSL.

### 1. Установка OpenSSL
- Скачать инсталлятор: [Win32/Win64 OpenSSL](https://slproweb.com/products/Win32OpenSSL.html)  
- Установить в стандартный путь:  
  ```
  C:\Program Files\OpenSSL-Win64
  ```

### 2. Настройка переменной окружения
Создать переменную окружения `OPENSSL_PATH`:
```powershell
setx OPENSSL_PATH "C:\Program Files\OpenSSL-Win64"
```

После этого **перезапустить Qt Creator** или консоль.

### 3. Использование в проекте (qmake)
В `.pro` файле:

```qmake
win32 {
    OPENSSL_PATH = $$(OPENSSL_PATH)

    INCLUDEPATH += "$$OPENSSL_PATH/include"
    LIBS += -L"$$OPENSSL_PATH/lib/VC/x64/MD"
    LIBS += -llibssl -llibcrypto
}
```
